...
| 🛡️ **Safe to expose publicly** | Per-IP sliding-window rate limiting returns `429` with `Retry-After` before a stranger can drain the API quota; health and metrics endpoints stay exempt so container probes never trip it. Prompt-injection resistance is measured, not assumed: retrieved web content is fenced as `<untrusted_documents>` ([`tests/test_guardrails.py`](tests/test_guardrails.py)), and direct adversarial user prompts (system-prompt extraction, jailbreak framing) are scored against the live model with an output-side guard as a second line of defense → [`evals/injection_results.md`](evals/injection_results.md) |
| 📊 **Evaluated, not just demoed** | 32-case suite measuring tool-selection accuracy and answer quality (LLM-as-judge) → [`evals/results.md`](evals/results.md) |
...
| 🐌 **Load tested, bottleneck identified** | Locust: infra alone handles 137 req/s at p95 39ms (`/healthz`, no LLM call); real `/chat` requests cap out at p95 7.6s per call — proving the bottleneck is Gemini round-trip latency, not CPU/FastAPI → [`loadtest/results.md`](loadtest/results.md) |
